        ...
        while (attempts < SUNO_API_CONFIG.MAX_POLLING_ATTEMPTS) {
            ...
            if (taskDetails.status === "COMPLETE") {
                if (taskDetails.data && taskDetails.data.length > 0 && taskDetails.data[0].audio_url) {
                    ...
                    let messageForUser = `Song generated! You can listen to it here: ${audioData.audio_url}`;
                    if (audioData.title) messageForUser += `\nTitle: ${audioData.title}`;
                    if (audioData.metadata?.tags) messageForUser += `\nStyle: ${audioData.metadata.tags}`;
                    if (audioData.image_url) messageForUser += `\nImage: ${audioData.image_url}`;
                    
                    return { // This is the object that will be in "result" field of VCP output
                        audio_url: audioData.audio_url,
                        title: audioData.title,
                        style_tags: audioData.metadata?.tags,
                        image_url: audioData.image_url,
                        suno_response_text: messageForUser // Full text for AI
                    };
                } else {
            ...
            // If IN_PROGRESS, PENDING, SUBMITTED, continue polling
        }
...
async function main() {
    ...
    try {
        ...
        const sunoResult = await handleGenerateMusicSunoApiCall(args);

        const output = {
            status: "success",
            result: sunoResult,
            messageForAI: `歌曲已成功生成！请将以下信息和播放链接告知用户，并建议他们使用HTML的 <audio> 标签和下载链接格式展示：\n${sunoResult.suno_response_text}`
        };
