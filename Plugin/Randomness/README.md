...
1.  **自定义骰面**: `Xd{面1,面2,...}`
    -   描述: 从给定的列表中随机抽取 X 个结果。
    -   示例: `1d{攻击,防御,闪避}`
    -   **注意**: 自定义骰面不支持其他任何修正指令（如加减、取高低等）。

2.  **Fate/Fudge 骰子**: `XdF`
    -   描述: 投掷 X 个 Fate/Fudge 骰子，每个骰子的面值为 `+`, `-`, ` ` (空白)，分别代表 `+1`, `-1`, `0`。
    -   示例: `4dF`
    -   **注意**: Fate 骰子不支持其他任何修正指令。

3.  **重复掷骰 (Repeat)**: `(次数)r(表达式)`
    -   描述: 重复执行一个掷骰表达式 N 次。
...
9.  **D&D 5e 优势/劣势**: `adv` 或 `dis`
    -   描述: `1d20adv` 等同于 `2d20kh1` (优势)，`1d20dis` 等同于 `2d20kl1` (劣势)。
    -   示例: `1d20adv+5`

10. **COC 奖励/惩罚骰**: `bp(数量)` 或 `pb(数量)`
