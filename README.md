...
- **AI 自主记忆与管理**: Agent 能够通过日记本（`DailyNote`）实现完全自主的记忆管理。这包括自我写入、编辑、整理、添加 Tag 标记、追踪 WorkID，甚至将知识分享给人类或其他 Agent，从而构建起一个动态演化、跨模型的共享知识库。
...
- **高级日记库架构**:
    - **双向一致性存储**: 所有日记以 `.MD/.txt` 格式存储于 Agent 的主记忆区与知识分区，便于前端渲染和管理。后端则构建了一个对等的 4096 维向量化数据库。
    - **原子级即时同步**: 向量数据库通过监控日记文件夹的哈希值变动，进行精细化的差分更新。这确保了用户对日记文件的任何微小修改（哪怕是一个字），都会被即时地、原子级地同步到向量数据库中（基于哈希值、时间戳和 Base64 字段）。
    - **企业级鲁棒性**: 系统内置了强大的多边同步机制、多重区块化 RAG 管理与备份、自动抗崩溃能力、数据库损毁自修复镜像以及版本回溯功能，确保记忆数据的高可用与高可靠性。
- **深度回忆**: Agent 可以自主查找和匹配所有属于自己的客户端聊天历史，精准回忆起具体的聊天片段，实现真正的“记忆回溯”。

#### 记忆召回机制的革命

为了实现对记忆的精准、高效、智能化调用，VCP 设计了三种不同的日记声明模式，并辅以先进的召回算法：

- **主日记挂载 (`{{角色名日记本}}`)**:
  - **声明方式**: `{{小克日常日记本}}`
  - **工作机制**: 这是标准的记忆注入模式，默认挂载指定日记本的所有分区内容，形成一个完整的背景上下文。

- **动态 RAG 注入 (`[[角色名日记本]]`)**:
  - **声明方式**: `[[小克物理日记本]]` 或 `[[小克物理日记本:1.5]]`
  - **工作机制**: 启用基于 VCP 原创多语言 RAG 算法的动态召回。系统会实时分析当前对话的上下文，智能地从指定的日记本中检索最相关的知识片段进行注入，而非加载全文。
  - **高级用法**: 用户可以通过在声明后附加 `:乘数` (如 `:1.5`, `:0.8`) 的方式，精确地调整该日记本的检索数量（k值）。例如，`:1.5` 会将算法计算出的基础k值乘以1.5倍，从而检索更多内容。这为同时使用多个RAG知识库并希望突出某个库的权重时提供了极大的灵活性。
  - **优势**: 极大优化了长篇日记或知识库的使用效率，减少了无关信息的干扰和 token 消耗，使上下文注入更精准、更可控。

- **条件语义注入 (`<<角色名日记本>>`)**:
  - **声明方式**: `<<VCP开发进度日记本>>`
  - **工作机制**: 一种基于聊天环境进行条件判断的完整注入模式。用户可以设计自然语言触发条件（如特定关键词、主题），当满足条件时，VCP 会将该日记本的全部内容完整载入上下文。
  - **优势**: 提供了更精细的控制逻辑。例如，只有在讨论 VCP 开发时，才会完整注入 `<<VCP开发进度日记本>>`，避免在日常对话中被庞大的专业知识库所累。

- **次时代高级召回算法**:
  - **二维动态决策**: VCP 的召回算法远超传统的相似度搜索。它首先会分析当前对话的话题复杂度、广度和深度，从而动态决定需要从最近的对话历史中向量化多少内容。
  - **多维返回策略**: 接着，它会从用户需求的复杂度、AI 输出的宽泛度这两个维度，来智能决策应该从每个挂载的知识库中返回多少条 RAG 知识。
  - **动态权重调整**: 用户可以通过 `[[日记本名:乘数]]` 语法，为每个挂载的 RAG 数据库独立设置检索数量（k值）的乘数。这使得在单次查询中，可以精确地让某些知识库返回更多或更少的结果，实现了用户级的动态权重控制。
