...
- **核心能力**:
    - **配置驱动**: 在 `config.env` 中通过 `WhitelistModel` 变量配置一个或多个需要“穿透”的模型名称（用逗号分隔）。
    - **请求绕行**: 当服务器收到一个针对白名单内模型的请求时，它会**完全跳过**所有的常规处理步骤（包括消息预处理、变量替换、工具调用循环等）。
    - **直接转发**: 请求体和响应体将在客户端与上游API之间被原封不动地直接转发。
