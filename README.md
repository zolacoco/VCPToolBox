...
```
系统工具列表：
- Flux 风格图片生成器 (FluxGen) - 命令: N/A:
    调用此工具通过SiliconFlow API的FLUX.1-schnell模型生成图片。请在您的回复中，使用以下精确格式来请求图片生成，确保所有参数值都用「始」和「末」准确包裹：
    <<<[TOOL_REQUEST]>>>
...
- **``**: 当出现在用户消息或系统提示词中时，`ImageProcessor` 插件将被跳过，允许 AI 直接处理原始 Base64 图像数据（例如，使用具有原生多模态理解能力的模型时）。
